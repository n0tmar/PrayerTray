namespace PrayerTray.Localization;

internal static class LocalizationCatalog
{
    public static IReadOnlyList<string> SupportedLanguages { get; } = ["en", "ar", "fr", "ur", "tr", "id"];

    private static readonly IReadOnlyDictionary<string, string> English = new Dictionary<string, string>
    {
        ["AppName"] = "PrayerTray",
        ["Settings"] = "Settings",
        ["PrayerTimes"] = "Prayer times",
        ["Location"] = "Location",
        ["Location_Format"] = "Location: {0}",
        ["Language"] = "Language",
        ["General"] = "General",
        ["ManualLocation"] = "Set location manually",
        ["City"] = "City",
        ["Taskbar"] = "Taskbar",
        ["TaskbarContentMode"] = "Taskbar text",
        ["TaskbarContentMode_time"] = "Prayer time",
        ["TaskbarContentMode_countdown"] = "Countdown",
        ["TaskbarContentMode_smart"] = "Smart",
        ["FooterMadeByMar"] = "Made with care by mar.",
        ["FooterSupportProject"] = "Support on Patreon",
        ["StartWithWindows"] = "Start with Windows",
        ["ShowWidget"] = "Show prayer time on taskbar",
        ["ShowNotificationIcon"] = "Show tray icon",
        ["Cancel"] = "Cancel",
        ["Save"] = "Save",
        ["Refresh"] = "Refresh",
        ["Detecting"] = "Detecting...",
        ["Unavailable"] = "Unavailable",
        ["UnknownLocation"] = "Unknown location",
        ["ChooseCity"] = "Search for a city.",
        ["SettingsSaved"] = "Saved.",
        ["Prayer"] = "Prayer",
        ["Prayer_Fajr"] = "Fajr",
        ["Prayer_Dhuhr"] = "Dhuhr",
        ["Prayer_Asr"] = "Asr",
        ["Prayer_Maghrib"] = "Maghrib",
        ["Prayer_Isha"] = "Isha",
        ["Tray_TodaySchedule"] = "Today's schedule",
        ["Tray_ShowWidget"] = "Show widget",
        ["Tray_HideWidget"] = "Hide widget",
        ["Tray_RefreshLocation"] = "Refresh location",
        ["Tray_Settings"] = "Settings",
        ["Tray_Exit"] = "Exit",
        ["Countdown_Seconds"] = "{0}s",
        ["Countdown_Minutes"] = "{0}m",
        ["Countdown_Hours"] = "{0}h",
        ["Countdown_HoursMinutes"] = "{0}h {1}m",
        ["CalcMethod_1"] = "University of Islamic Sciences, Karachi",
        ["CalcMethod_2"] = "ISNA (North America)",
        ["CalcMethod_3"] = "Muslim World League",
        ["CalcMethod_4"] = "Umm Al-Qura, Makkah",
        ["CalcMethod_5"] = "Egyptian General Authority",
        ["CalcMethod_7"] = "Institute of Geophysics, Tehran",
        ["CalcMethod_8"] = "Gulf Region",
        ["CalcMethod_9"] = "Kuwait",
        ["CalcMethod_10"] = "Qatar",
        ["CalcMethod_13"] = "Diyanet, Turkey",
        ["CalcMethod_17"] = "JAKIM, Malaysia",
        ["CalcMethod_20"] = "Kemenag, Indonesia",
        ["Lang_en"] = "English",
        ["Lang_ar"] = "العربية",
        ["Lang_fr"] = "Français",
        ["Lang_ur"] = "اردو",
        ["Lang_tr"] = "Türkçe",
        ["Lang_id"] = "Bahasa Indonesia"
    };

    private static readonly IReadOnlyDictionary<string, string> Arabic = new Dictionary<string, string>
    {
        ["AppName"] = "مواقيت الصلاة",
        ["Settings"] = "الإعدادات",
        ["PrayerTimes"] = "مواقيت الصلاة",
        ["Location"] = "الموقع",
        ["Location_Format"] = "الموقع: {0}",
        ["Language"] = "اللغة",
        ["General"] = "عام",
        ["ManualLocation"] = "تعيين الموقع يدويًا",
        ["City"] = "المدينة",
        ["Taskbar"] = "شريط المهام",
        ["TaskbarContentMode"] = "نص شريط المهام",
        ["TaskbarContentMode_time"] = "وقت الصلاة",
        ["TaskbarContentMode_countdown"] = "عد تنازلي",
        ["TaskbarContentMode_smart"] = "ذكي",
        ["FooterMadeByMar"] = "صنعه mar بعناية.",
        ["FooterSupportProject"] = "ادعم عبر Patreon",
        ["StartWithWindows"] = "البدء مع Windows",
        ["ShowWidget"] = "إظهار وقت الصلاة على شريط المهام",
        ["ShowNotificationIcon"] = "إظهار أيقونة الظل",
        ["Cancel"] = "إلغاء",
        ["Save"] = "حفظ",
        ["Refresh"] = "تحديث",
        ["Detecting"] = "جارٍ التحديد...",
        ["Unavailable"] = "غير متاح",
        ["UnknownLocation"] = "موقع غير معروف",
        ["ChooseCity"] = "ابحث عن مدينة.",
        ["SettingsSaved"] = "تم الحفظ.",
        ["Prayer"] = "صلاة",
        ["Prayer_Fajr"] = "الفجر",
        ["Prayer_Dhuhr"] = "الظهر",
        ["Prayer_Asr"] = "العصر",
        ["Prayer_Maghrib"] = "المغرب",
        ["Prayer_Isha"] = "العشاء",
        ["Tray_TodaySchedule"] = "جدول اليوم",
        ["Tray_ShowWidget"] = "إظهار العنصر",
        ["Tray_HideWidget"] = "إخفاء العنصر",
        ["Tray_RefreshLocation"] = "تحديث الموقع",
        ["Tray_Settings"] = "الإعدادات",
        ["Tray_Exit"] = "خروج",
        ["Countdown_Seconds"] = "{0} ث",
        ["Countdown_Minutes"] = "{0} د",
        ["Countdown_Hours"] = "{0} س",
        ["Countdown_HoursMinutes"] = "{0} س {1} د",
        ["CalcMethod_1"] = "جامعة العلوم الإسلامية، كراتشي",
        ["CalcMethod_2"] = "ISNA (أمريكا الشمالية)",
        ["CalcMethod_3"] = "رابطة العالم الإسلامي",
        ["CalcMethod_4"] = "أم القرى، مكة",
        ["CalcMethod_5"] = "الهيئة المصرية العامة",
        ["CalcMethod_7"] = "معهد الجيوفيزياء، طهران",
        ["CalcMethod_8"] = "منطقة الخليج",
        ["CalcMethod_9"] = "الكويت",
        ["CalcMethod_10"] = "قطر",
        ["CalcMethod_13"] = "ديانت، تركيا",
        ["CalcMethod_17"] = "JAKIM، ماليزيا",
        ["CalcMethod_20"] = "Kemenag، إندونيسيا",
        ["Lang_en"] = "English",
        ["Lang_ar"] = "العربية",
        ["Lang_fr"] = "Français",
        ["Lang_ur"] = "اردو",
        ["Lang_tr"] = "Türkçe",
        ["Lang_id"] = "Bahasa Indonesia"
    };

    private static readonly IReadOnlyDictionary<string, string> French = new Dictionary<string, string>
    {
        ["AppName"] = "PrayerTray",
        ["Settings"] = "Paramètres",
        ["PrayerTimes"] = "Heures de prière",
        ["Location"] = "Emplacement",
        ["Location_Format"] = "Emplacement : {0}",
        ["Language"] = "Langue",
        ["General"] = "Général",
        ["ManualLocation"] = "Définir la position manuellement",
        ["City"] = "Ville",
        ["Taskbar"] = "Barre des tâches",
        ["TaskbarContentMode"] = "Texte de la barre des tâches",
        ["TaskbarContentMode_time"] = "Heure de prière",
        ["TaskbarContentMode_countdown"] = "Compte à rebours",
        ["TaskbarContentMode_smart"] = "Intelligent",
        ["FooterMadeByMar"] = "Créé avec soin par mar.",
        ["FooterSupportProject"] = "Soutenir sur Patreon",
        ["StartWithWindows"] = "Démarrer avec Windows",
        ["ShowWidget"] = "Afficher l'heure de prière dans la barre des tâches",
        ["ShowNotificationIcon"] = "Afficher l'icône",
        ["Cancel"] = "Annuler",
        ["Save"] = "Enregistrer",
        ["Refresh"] = "Actualiser",
        ["Detecting"] = "Détection...",
        ["Unavailable"] = "Indisponible",
        ["UnknownLocation"] = "Emplacement inconnu",
        ["ChooseCity"] = "Recherchez une ville.",
        ["SettingsSaved"] = "Enregistré.",
        ["Prayer"] = "Prière",
        ["Prayer_Fajr"] = "Fajr",
        ["Prayer_Dhuhr"] = "Dhuhr",
        ["Prayer_Asr"] = "Asr",
        ["Prayer_Maghrib"] = "Maghrib",
        ["Prayer_Isha"] = "Isha",
        ["Tray_TodaySchedule"] = "Horaire du jour",
        ["Tray_ShowWidget"] = "Afficher le widget",
        ["Tray_HideWidget"] = "Masquer le widget",
        ["Tray_RefreshLocation"] = "Actualiser l'emplacement",
        ["Tray_Settings"] = "Paramètres",
        ["Tray_Exit"] = "Quitter",
        ["Countdown_Seconds"] = "{0}s",
        ["Countdown_Minutes"] = "{0}m",
        ["Countdown_Hours"] = "{0}h",
        ["Countdown_HoursMinutes"] = "{0}h {1}m",
        ["CalcMethod_1"] = "Université des sciences islamiques, Karachi",
        ["CalcMethod_2"] = "ISNA (Amérique du Nord)",
        ["CalcMethod_3"] = "Ligue du monde musulman",
        ["CalcMethod_4"] = "Umm Al-Qura, La Mecque",
        ["CalcMethod_5"] = "Autorité égyptienne générale",
        ["CalcMethod_7"] = "Institut de géophysique, Téhéran",
        ["CalcMethod_8"] = "Région du Golfe",
        ["CalcMethod_9"] = "Koweït",
        ["CalcMethod_10"] = "Qatar",
        ["CalcMethod_13"] = "Diyanet, Turquie",
        ["CalcMethod_17"] = "JAKIM, Malaisie",
        ["CalcMethod_20"] = "Kemenag, Indonésie",
        ["Lang_en"] = "English",
        ["Lang_ar"] = "العربية",
        ["Lang_fr"] = "Français",
        ["Lang_ur"] = "اردو",
        ["Lang_tr"] = "Türkçe",
        ["Lang_id"] = "Bahasa Indonesia"
    };

    private static readonly IReadOnlyDictionary<string, string> Urdu = new Dictionary<string, string>
    {
        ["AppName"] = "PrayerTray",
        ["Settings"] = "ترتیبات",
        ["PrayerTimes"] = "نماز کے اوقات",
        ["Location"] = "مقام",
        ["Location_Format"] = "مقام: {0}",
        ["Language"] = "زبان",
        ["General"] = "عمومی",
        ["ManualLocation"] = "مقام دستی طور پر سیٹ کریں",
        ["City"] = "شہر",
        ["Taskbar"] = "ٹاسک بار",
        ["TaskbarContentMode"] = "ٹاسک بار متن",
        ["TaskbarContentMode_time"] = "نماز کا وقت",
        ["TaskbarContentMode_countdown"] = "الٹی گنتی",
        ["TaskbarContentMode_smart"] = "سمارٹ",
        ["FooterMadeByMar"] = "mar نے اسے محبت سے بنایا۔",
        ["FooterSupportProject"] = "Patreon پر مدد کریں",
        ["StartWithWindows"] = "ونڈوز کے ساتھ شروع",
        ["ShowWidget"] = "ٹاسک بار پر نماز کا وقت دکھائیں",
        ["ShowNotificationIcon"] = "ٹری آئیکن دکھائیں",
        ["Cancel"] = "منسوخ",
        ["Save"] = "محفوظ",
        ["Refresh"] = "تازہ کریں",
        ["Detecting"] = "تلاش جاری...",
        ["Unavailable"] = "دستیاب نہیں",
        ["UnknownLocation"] = "نامعلوم مقام",
        ["ChooseCity"] = "شہر تلاش کریں۔",
        ["SettingsSaved"] = "محفوظ ہو گیا۔",
        ["Prayer"] = "نماز",
        ["Prayer_Fajr"] = "فجر",
        ["Prayer_Dhuhr"] = "ظہر",
        ["Prayer_Asr"] = "عصر",
        ["Prayer_Maghrib"] = "مغرب",
        ["Prayer_Isha"] = "عشاء",
        ["Tray_TodaySchedule"] = "آج کا شیڈول",
        ["Tray_ShowWidget"] = "ویجٹ دکھائیں",
        ["Tray_HideWidget"] = "ویجٹ چھپائیں",
        ["Tray_RefreshLocation"] = "مقام تازہ کریں",
        ["Tray_Settings"] = "ترتیبات",
        ["Tray_Exit"] = "بند کریں",
        ["Countdown_Seconds"] = "{0} س",
        ["Countdown_Minutes"] = "{0} م",
        ["Countdown_Hours"] = "{0} گ",
        ["Countdown_HoursMinutes"] = "{0} گ {1} م",
        ["CalcMethod_1"] = "یونیورسٹی آف اسلامی سائنسز، کراچی",
        ["CalcMethod_2"] = "ISNA (شمالی امریکہ)",
        ["CalcMethod_3"] = "مسلم ورلڈ لیگ",
        ["CalcMethod_4"] = "ام القری، مکہ",
        ["CalcMethod_5"] = "مصری جنرل اتھارٹی",
        ["CalcMethod_7"] = "انسٹی ٹیوٹ آف جیو فزکس، تہران",
        ["CalcMethod_8"] = "خلیجی خطہ",
        ["CalcMethod_9"] = "کویت",
        ["CalcMethod_10"] = "قطر",
        ["CalcMethod_13"] = "دیانت، ترکی",
        ["CalcMethod_17"] = "JAKIM، ملائیشیا",
        ["CalcMethod_20"] = "Kemenag، انڈونیشیا",
        ["Lang_en"] = "English",
        ["Lang_ar"] = "العربية",
        ["Lang_fr"] = "Français",
        ["Lang_ur"] = "اردو",
        ["Lang_tr"] = "Türkçe",
        ["Lang_id"] = "Bahasa Indonesia"
    };

    private static readonly IReadOnlyDictionary<string, string> Turkish = new Dictionary<string, string>
    {
        ["AppName"] = "PrayerTray",
        ["Settings"] = "Ayarlar",
        ["PrayerTimes"] = "Namaz vakitleri",
        ["Location"] = "Konum",
        ["Location_Format"] = "Konum: {0}",
        ["Language"] = "Dil",
        ["General"] = "Genel",
        ["ManualLocation"] = "Konumu elle ayarla",
        ["City"] = "Şehir",
        ["Taskbar"] = "Görev çubuğu",
        ["TaskbarContentMode"] = "Görev çubuğu metni",
        ["TaskbarContentMode_time"] = "Namaz vakti",
        ["TaskbarContentMode_countdown"] = "Geri sayım",
        ["TaskbarContentMode_smart"] = "Akıllı",
        ["FooterMadeByMar"] = "mar tarafından özenle yapıldı.",
        ["FooterSupportProject"] = "Patreon’da destekle",
        ["StartWithWindows"] = "Windows ile başlat",
        ["ShowWidget"] = "Görev çubuğunda namaz vaktini göster",
        ["ShowNotificationIcon"] = "Tepsi simgesini göster",
        ["Cancel"] = "İptal",
        ["Save"] = "Kaydet",
        ["Refresh"] = "Yenile",
        ["Detecting"] = "Algılanıyor...",
        ["Unavailable"] = "Kullanılamıyor",
        ["UnknownLocation"] = "Bilinmeyen konum",
        ["ChooseCity"] = "Şehir ara.",
        ["SettingsSaved"] = "Kaydedildi.",
        ["Prayer"] = "Namaz",
        ["Prayer_Fajr"] = "İmsak",
        ["Prayer_Dhuhr"] = "Öğle",
        ["Prayer_Asr"] = "İkindi",
        ["Prayer_Maghrib"] = "Akşam",
        ["Prayer_Isha"] = "Yatsı",
        ["Tray_TodaySchedule"] = "Bugünün programı",
        ["Tray_ShowWidget"] = "Widget göster",
        ["Tray_HideWidget"] = "Widget gizle",
        ["Tray_RefreshLocation"] = "Konumu yenile",
        ["Tray_Settings"] = "Ayarlar",
        ["Tray_Exit"] = "Çıkış",
        ["Countdown_Seconds"] = "{0} sn",
        ["Countdown_Minutes"] = "{0} dk",
        ["Countdown_Hours"] = "{0} sa",
        ["Countdown_HoursMinutes"] = "{0} sa {1} dk",
        ["CalcMethod_1"] = "İslami Bilimler Üniversitesi, Karaçi",
        ["CalcMethod_2"] = "ISNA (Kuzey Amerika)",
        ["CalcMethod_3"] = "Müslüman Dünya Birliği",
        ["CalcMethod_4"] = "Ümmü'l-Kura, Mekke",
        ["CalcMethod_5"] = "Mısır Genel Kurumu",
        ["CalcMethod_7"] = "Jeofizik Enstitüsü, Tahran",
        ["CalcMethod_8"] = "Körfez Bölgesi",
        ["CalcMethod_9"] = "Kuveyt",
        ["CalcMethod_10"] = "Katar",
        ["CalcMethod_13"] = "Diyanet, Türkiye",
        ["CalcMethod_17"] = "JAKIM, Malezya",
        ["CalcMethod_20"] = "Kemenag, Endonezya",
        ["Lang_en"] = "English",
        ["Lang_ar"] = "العربية",
        ["Lang_fr"] = "Français",
        ["Lang_ur"] = "اردو",
        ["Lang_tr"] = "Türkçe",
        ["Lang_id"] = "Bahasa Indonesia"
    };

    private static readonly IReadOnlyDictionary<string, string> Indonesian = new Dictionary<string, string>
    {
        ["AppName"] = "PrayerTray",
        ["Settings"] = "Pengaturan",
        ["PrayerTimes"] = "Jadwal shalat",
        ["Location"] = "Lokasi",
        ["Location_Format"] = "Lokasi: {0}",
        ["Language"] = "Bahasa",
        ["General"] = "Umum",
        ["ManualLocation"] = "Atur lokasi secara manual",
        ["City"] = "Kota",
        ["Taskbar"] = "Bilah tugas",
        ["TaskbarContentMode"] = "Teks bilah tugas",
        ["TaskbarContentMode_time"] = "Waktu shalat",
        ["TaskbarContentMode_countdown"] = "Hitung mundur",
        ["TaskbarContentMode_smart"] = "Cerdas",
        ["FooterMadeByMar"] = "Dibuat dengan sepenuh hati oleh mar.",
        ["FooterSupportProject"] = "Dukung di Patreon",
        ["StartWithWindows"] = "Mulai dengan Windows",
        ["ShowWidget"] = "Tampilkan waktu shalat di bilah tugas",
        ["ShowNotificationIcon"] = "Tampilkan ikon baki",
        ["Cancel"] = "Batal",
        ["Save"] = "Simpan",
        ["Refresh"] = "Segarkan",
        ["Detecting"] = "Mendeteksi...",
        ["Unavailable"] = "Tidak tersedia",
        ["UnknownLocation"] = "Lokasi tidak diketahui",
        ["ChooseCity"] = "Cari kota.",
        ["SettingsSaved"] = "Disimpan.",
        ["Prayer"] = "Shalat",
        ["Prayer_Fajr"] = "Subuh",
        ["Prayer_Dhuhr"] = "Dzuhur",
        ["Prayer_Asr"] = "Ashar",
        ["Prayer_Maghrib"] = "Maghrib",
        ["Prayer_Isha"] = "Isya",
        ["Tray_TodaySchedule"] = "Jadwal hari ini",
        ["Tray_ShowWidget"] = "Tampilkan widget",
        ["Tray_HideWidget"] = "Sembunyikan widget",
        ["Tray_RefreshLocation"] = "Segarkan lokasi",
        ["Tray_Settings"] = "Pengaturan",
        ["Tray_Exit"] = "Keluar",
        ["Countdown_Seconds"] = "{0} d",
        ["Countdown_Minutes"] = "{0} m",
        ["Countdown_Hours"] = "{0} j",
        ["Countdown_HoursMinutes"] = "{0} j {1} m",
        ["CalcMethod_1"] = "Universitas Ilmu Islam, Karachi",
        ["CalcMethod_2"] = "ISNA (Amerika Utara)",
        ["CalcMethod_3"] = "Liga Muslim Dunia",
        ["CalcMethod_4"] = "Umm Al-Qura, Makkah",
        ["CalcMethod_5"] = "Otoritas Umum Mesir",
        ["CalcMethod_7"] = "Institut Geofisika, Tehran",
        ["CalcMethod_8"] = "Wilayah Teluk",
        ["CalcMethod_9"] = "Kuwait",
        ["CalcMethod_10"] = "Qatar",
        ["CalcMethod_13"] = "Diyanet, Turki",
        ["CalcMethod_17"] = "JAKIM, Malaysia",
        ["CalcMethod_20"] = "Kemenag, Indonesia",
        ["Lang_en"] = "English",
        ["Lang_ar"] = "العربية",
        ["Lang_fr"] = "Français",
        ["Lang_ur"] = "اردو",
        ["Lang_tr"] = "Türkçe",
        ["Lang_id"] = "Bahasa Indonesia"
    };

    public static IReadOnlyDictionary<string, IReadOnlyDictionary<string, string>> All { get; } =
        new Dictionary<string, IReadOnlyDictionary<string, string>>(StringComparer.OrdinalIgnoreCase)
        {
            ["en"] = English,
            ["ar"] = Arabic,
            ["fr"] = French,
            ["ur"] = Urdu,
            ["tr"] = Turkish,
            ["id"] = Indonesian
        };
}
